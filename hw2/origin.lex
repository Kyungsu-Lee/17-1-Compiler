...
(this|super)({WP}|[(]|[.])	{
	file = fopen(output, "a");
					...
					printf("<div class='super'>%s</div>%c", tmp, yytext[yyleng - 1]);
					fprintf(file, "<div class='super'>%s</div>%c", tmp, yytext[yyleng - 1]);
	fclose(file);
				}
...
({WP}|{BR})(this|super)({WP}|[(]|[.])	{
	file = fopen(output, "a");
					...
					printf("%c<div class='super'>%s</div>%c", yytext[0], tmp, yytext[yyleng - 1]);
					fprintf(file,"%c<div class='super'>%s</div>%c", yytext[0], tmp, yytext[yyleng - 1]);
	fclose(file);
				}
...
(static){WP}	{
	file = fopen(output, "a");
	printf("<b>%s</b>", yytext);
	fprintf(file,"<div class='c1'>%s</div>", yytext);
	fclose(file);
}
{AD}(static){WP} {
	file = fopen(output, "a");
...
({WP}|{BR}|[(])(static){WP} {
	file = fopen(output, "a");
					...
					strncpy(tmp, yytext + 1, yyleng - 2);
					printf("%c<b>%s</b>%c", yytext[0], tmp, yytext[yyleng - 1]);
					fprintf(file,"%c<div class='c1'>%s</div>%c", yytext[0], tmp, yytext[yyleng - 1]);
	fclose(file);
				}
	/*	class, interface	*/
({OJ}){WP}	{
	...
	char **argv;
{
	...
	fprintf(fp, "<head>\n");
	fprintf(fp, "<style>\n");
	fprintf(fp, ".nnb{color : #FF0000}\n");
	fprintf(fp, ".type{color : #FF0000}\n");
	fprintf(fp, ".numtype{color : #FF0000}\n");
	fprintf(fp, ".mod{color : #FF0000}\n");
	fprintf(fp, ".fs{color : #FF0000}\n");
	fprintf(fp, ".loop{color : #FF0000}\n");
	fprintf(fp, ".oj{color : #FF0000}\n");
	fprintf(fp, ".cond{color : #FF0000}\n");
	fprintf(fp, ".super{color : #FF0000}\n");
	fprintf(fp, "</style>\n");
	fprintf(fp, "</head>\n");
	fprintf(fp, "<body>\n<pre>\n");
